...
export function ActivityFeed({ items }: ActivityFeedProps): JSX.Element {
  ...
  return (
    <section className="rounded-lg border border-border bg-white p-4 shadow-sm" aria-label="Activity feed">
      ...
      {items.length === 0 ? (
        ...
        <div className="mt-4 relative pl-4 border-l border-gray-100 space-y-4">
          {items.map((item) => {
            ...
            return (
              <div key={item.id} className="relative group">
                ...
                <div
                  className={cn(
                    "absolute -left-[21px] top-1.5 h-2.5 w-2.5 rounded-full ring-4 bg-white transition-all duration-300",
                    dotColor,
                  )}
                />
